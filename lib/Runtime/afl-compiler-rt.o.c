...
static void __afl_start_forkserver(void) {
...
#ifdef __linux__
  if (!getenv("AFL_NO_SNAPSHOT") && afl_snapshot_init() >= 0) {
  ...
  if (__afl_sharedmem_fuzzing) {
    status_for_fsrv |= FS_OPT_SHDMEM_FUZZ;
  }
  if (status_for_fsrv) {
...
void __afl_manual_init(void) {
  ...
  if (getenv("AFL_DISABLE_LLVM_INSTRUMENTATION")) {
    ...
    if (__afl_debug) {
      fprintf(stderr, "DEBUG: disabled instrumentation because of "
...
void __path_profiler_dbg(FTEntry *entry, u32 path_num) {
  ...
  fprintf(stderr,
          ...
          "path: %u)\n",
          counter, entry, entry->size, path_num);
}

void __path_profiler_init(FTEntry *start, FTEntry *stop) {
  static u32 afl_idx = 0;
  static u32 num_paths = 0;
  static u32 max_num_entries = 0;
  ...
  __afl_unmap_shm();
  __afl_final_loc = num_paths;
  __afl_map_shm();

  for (FTEntry *entry = start; entry < stop; ++entry) {
    entry->array = __afl_area_ptr + afl_idx;
    afl_idx += entry->size;
  }
}
...
void __afl_coverage_interesting(u8 val, u32 id) { __afl_area_ptr[id] = val; }

void __afl_set_persistent_mode(u8 mode) { is_persistent = mode; }
