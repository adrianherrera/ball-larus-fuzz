...
void BallLarusDag::calculatePathNumbers() {
  ...
  while (bfsQueue.size() > 0) {
    ...
    LLVM_DEBUG(dbgs() << "calculatePathNumbers on " << node->getName() << "\n");
    ...
    LLVM_DEBUG(dbgs() << "prev, new number paths " << prevPathNumber << ", "
                      << node->getNumberPaths() << ".\n");

    if (prevPathNumber == 0 && node->getNumberPaths() != 0) {
      LLVM_DEBUG(dbgs() << "node ready : " << node->getName() << "\n");
  ...
  LLVM_DEBUG(dbgs() << "\tNumber of paths: " << getRoot()->getNumberPaths()
                    << "\n");
}
...
void BallLarusDag::buildEdge(BLBlockNodeMap &inDag,
  ...
  if (succNode && succNode->getColor() == BallLarusNode::BLACK) {
    ...
    LLVM_DEBUG(dbgs() << "Backedge detected.\n");
